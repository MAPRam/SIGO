    ...
    else
    {
        ...
        if ($tmpFilePath != "")
        {
            ...
            if (move_uploaded_file($tmpFilePath, $fil))
            {
                ...
                $cadena1 = 'INSERT INTO mensajes(id_mensaje_send, id_usuario_send, id_usuario_get, titulo_mensaje, descripcion, fecha_enviado,hora_enviado, estado_respuesta, enlace_enviado, enlace_respuesta, mensaje_respuesta, hora_respuesta,compartido, fecha_respuesta, responde, prioridad)';
                $cadena2 = ' VALUES('. $datos2["maximo"] .', ' . $_SESSION["id_usuario"] . ', ' . $id_get . ', "' . $_POST["titulo"] .'", "' . $_POST["describe"] . '", "' . $fecha2 .'", "' . date("H:i") .
                '" , "0", "' . $fil . '" , "no contestado", "0", "0","0", "0", "0","'.$prioridad.'")';
